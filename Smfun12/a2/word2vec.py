...
def naiveSoftmaxLossAndGradient(
    ...
    dot = np.dot(outsideVectors[outsideWordIdx],centerWordVec)

    nom = np.exp(dot)

    den = np.sum(np.exp(np.matmul(outsideVectors, centerWordVec)))

    # loss = -np.log(softmax(dot))
    ...
    gradOutsideVecs = np.array([[1, 2], [3, 4]])

    ### END YOUR CODE


    return loss1, gradCenterVec, gradOutsideVecs
...
def negSamplingLossAndGradient(
    ...
    first = -np.log(sigmoid(np.dot(np.transpose(outsideVectors[outsideWordIdx]), centerWordVec)))
    second = 0
    for i in range(K+1):
        second += np.log(sigmoid(np.dot(-np.transpose(outsideVectors[i]), centerWordVec)))
