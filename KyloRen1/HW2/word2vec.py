...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = -outsideVectors[outsideWordIdx] + np.sum(
        np.multiply(y_hat.reshape(y_hat.shape[0], 1), outsideVectors), axis=0
    )
    gradOutsideVecs = np.multiply(centerWordVec, y_hat[:, np.newaxis])
...
def negSamplingLossAndGradient(
    ...
    y_hat[negSampleWordIndices] = - y_hat[negSampleWordIndices]
    y_hat = sigmoid(y_hat)

    loss = - np.log(y_hat[outsideWordIdx]) - np.sum(np.log(y_hat[negSampleWordIndices]))

    gradCenterVec = (y_hat[outsideWordIdx] - 1) * outsideVectors[outsideWordIdx] + np.sum(
        (outsideVectors[negSampleWordIndices].T * (1 - y_hat[negSampleWordIndices])).T, axis =0
    )
    
    gradOutsideVecs[outsideWordIdx] = centerWordVec * (y_hat[outsideWordIdx] - 1)
    gradOutsideVecs[negSampleWordIndices] = np.multiply(centerWordVec,(1 - y_hat[negSampleWordIndices])[:,np.newaxis])
    neg_count = np.bincount(negSampleWordIndices)
    neg_count[neg_count == 0] = 1
    gradOutsideVecs[:len(neg_count)] = (neg_count * gradOutsideVecs[:len(neg_count)].T).T
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    centerVector = centerWordVectors[word2Ind[currentCenterWord]]

    for word in outsideWords:
        curr_loss, grad_center, grad_outside = word2vecLossAndGradient(
            centerVector, word2Ind[word], outsideVectors, dataset)
        loss += curr_loss
        gradOutsideVectors += grad_outside
        gradCenterVecs[word2Ind[currentCenterWord]] += grad_center
